...
/*
 * QEMU virtual microphone shim.
 *
 * QEMU's esp32c3 machine does not emulate I2S/SPI PDM at all, so the real
 * SPI-driven PDM capture below would either error out or block forever.
 * To keep the dialog uplink path *exactly* the same as on real hardware
 * (BSP -> dialog_uplink_task -> conversation_client_send_audio -> nghttp2),
 * we replace only the leaf bsp_microphone_* symbols with a wall-clock-paced
 * generator that produces PCM16 mono @ 24000 Hz (matching the real
 * CONFIG_COLLAR_MICROPHONE_SAMPLE_RATE / CONFIG_COLLAR_MICROPHONE_STEREO=n
 * baseline). Everything above the BSP boundary - VAD timing, chunking,
 * gRPC framing - sees the same byte stream cadence it would on a device.
 *
 * Real-hardware builds (CONFIG_COLLAR_QEMU_OPENETH=n) skip this entire
 * block via #else and fall through to the unchanged PDM driver below.
 */
...
/*
 * Real-voice PCM fixture (24 kHz mono s16) generated by tools/gen_voice_pcm.py
 * via libespeak-ng. ~3.7 seconds of synthesized speech sandwiched in
 * 0.2 s leading + 1.0 s trailing silence so server-side VAD can segment
 * cleanly and the same fixture loops without hard discontinuities.
 *
 * The fixture is large (~235 KB), so it MUST live in flash rodata. The
 * `static const` declaration at file scope plus the array initializer in
 * the included header places it in `.rodata` automatically; on esp32c3
 * that ends up XIP-mapped from flash, not occupying internal RAM.
 *
 * Why not stick with the formant-tone synthesizer? Server-side STT (Whisper
 * / Riva) treats deterministic tone bursts as noise and never produces a
 * transcript, so the conversation pipeline never reaches audio_complete and
 * the multi-round flow stalls. With a real spoken phrase the same path
 * round-trips through STT -> LLM -> TTS in well under the 60 s round
 * timeout enforced by qemu_user_loop.
 */
#include "test_voice_pcm.h"

static const char *TAG = "mic";

#ifndef CONFIG_COLLAR_MICROPHONE_SAMPLE_RATE
#define CONFIG_COLLAR_MICROPHONE_SAMPLE_RATE 24000
#endif

#define BSP_QEMU_MIC_RATE_HZ ((uint32_t)CONFIG_COLLAR_MICROPHONE_SAMPLE_RATE)
#define BSP_QEMU_MIC_CHANNELS 1U

/*
 * "Voice-like" multi-tone synthesizer.
 *
 * The previous version produced a single 440 Hz sine, which a server-side
 * VAD treats as "constantly speaking" and which has none of the temporal
 * structure of a real utterance. We swap that for a deterministic but
 * speech-flavoured generator:
 *   - three formant-ish sinusoids (180 Hz f0 + 400 Hz + 900 Hz)
 *   - cosine-shaped envelope: 200 ms attack / 400 ms sustain /
 *     200 ms release, then 200 ms silence -> 1000 ms cycle, repeated
 *   - additive low-amplitude LCG noise (~3% of full scale) so spectrum
 *     is not a pure line spectrum
 *   - peak ~= -6 dBFS, roughly matching a real recording
 * Frame size / sample rate / channels are unchanged (PCM16 mono 24 kHz),
 * so dialog_uplink chunking, gRPC framing and packet sizes stay byte-for-
 * byte identical to a real-device "voice present" capture.
 */
#define BSP_QEMU_MIC_F0_HZ        180.0f
#define BSP_QEMU_MIC_F1_HZ        400.0f
#define BSP_QEMU_MIC_F2_HZ        900.0f
#define BSP_QEMU_MIC_PEAK         16000  /* ~ -6 dBFS */
#define BSP_QEMU_MIC_NOISE_AMPL   500    /* ~3% FS */
#define BSP_QEMU_MIC_CYCLE_MS     1000U
#define BSP_QEMU_MIC_ATTACK_MS    200U
#define BSP_QEMU_MIC_SUSTAIN_MS   400U
#define BSP_QEMU_MIC_RELEASE_MS   200U
/* SILENCE_MS = CYCLE - ATTACK - SUSTAIN - RELEASE = 200 ms */

static bool s_qemu_mic_inited;
static int64_t s_qemu_mic_start_us;
static uint64_t s_qemu_mic_samples_emitted;
static uint64_t s_qemu_mic_phase_samples;
static uint32_t s_qemu_mic_lcg = 0xC0FFEEu;

esp_err_t bsp_microphone_init(void)
{
    if (s_qemu_mic_inited) return ESP_OK;
    s_qemu_mic_start_us = esp_timer_get_time();
    s_qemu_mic_samples_emitted = 0U;
    s_qemu_mic_phase_samples = 0U;
    s_qemu_mic_inited = true;
    ESP_LOGI(TAG,
             "QEMU virtual mic up: PCM16 mono %u Hz voice-like (%.0f/%.0f/%.0f Hz) peak=%d cycle=%ums",
             (unsigned)BSP_QEMU_MIC_RATE_HZ,
             (double)BSP_QEMU_MIC_F0_HZ,
             (double)BSP_QEMU_MIC_F1_HZ,
             (double)BSP_QEMU_MIC_F2_HZ,
             (int)BSP_QEMU_MIC_PEAK,
             (unsigned)BSP_QEMU_MIC_CYCLE_MS);
    return ESP_OK;
}

void bsp_microphone_deinit(void)
{
    s_qemu_mic_inited = false;
    s_qemu_mic_samples_emitted = 0U;
    s_qemu_mic_phase_samples = 0U;
}

bool bsp_microphone_is_ready(void) { return s_qemu_mic_inited; }
uint32_t bsp_microphone_get_sample_rate_hz(void) { return BSP_QEMU_MIC_RATE_HZ; }
uint8_t bsp_microphone_get_channels(void) { return (uint8_t)BSP_QEMU_MIC_CHANNELS; }
size_t bsp_microphone_frame_bytes(void) { return BSP_QEMU_MIC_CHANNELS * sizeof(int16_t); }

static uint64_t qemu_mic_available_samples(void)
{
    int64_t now_us = esp_timer_get_time();
    if (now_us <= s_qemu_mic_start_us) return 0U;
    int64_t dt_us = now_us - s_qemu_mic_start_us;
    /* Use 64-bit math; rate * dt fits comfortably. */
    uint64_t total = ((uint64_t)dt_us * (uint64_t)BSP_QEMU_MIC_RATE_HZ) / 1000000ULL;
    if (total <= s_qemu_mic_samples_emitted) return 0U;
    return total - s_qemu_mic_samples_emitted;
}

static inline float qemu_mic_envelope(uint32_t cycle_ms)
{
    /*
     * 1000 ms cycle: 200 ms attack (cos rise) / 400 ms sustain / 200 ms
     * release (cos fall) / 200 ms silence. Continuous and non-negative.
     */
    if (cycle_ms < BSP_QEMU_MIC_ATTACK_MS) {
        float t = (float)cycle_ms / (float)BSP_QEMU_MIC_ATTACK_MS;
        return 0.5f * (1.0f - cosf(3.14159265f * t));
    }
    cycle_ms -= BSP_QEMU_MIC_ATTACK_MS;
    if (cycle_ms < BSP_QEMU_MIC_SUSTAIN_MS) {
        return 1.0f;
    }
    cycle_ms -= BSP_QEMU_MIC_SUSTAIN_MS;
    if (cycle_ms < BSP_QEMU_MIC_RELEASE_MS) {
        float t = (float)cycle_ms / (float)BSP_QEMU_MIC_RELEASE_MS;
        return 0.5f * (1.0f + cosf(3.14159265f * t));
    }
    return 0.0f;
}
...
/*
 * Per-tone phase accumulators kept *modulo 2π* so we never feed sinf() a
 * large argument. With s_qemu_mic_phase_samples being an unbounded uint64
 * the previous formulation `sinf(w * (float)n)` eventually passed values
 * in the tens of millions to sinf(); newlib's sinf falls back to
 * __kernel_rem_pio2f for any |x| >= 2^7, and on a soft-float RISC-V
 * target inside QEMU that costs hundreds of microseconds *per sample*.
 * On a 24 kHz / 1024-sample frame that wedged dlg_ul long enough to
 * trip the task watchdog (observed: panic'ed (Interrupt wdt timeout) /
 * MEPC inside __kernel_rem_pio2f / RA inside qemu_mic_fill).
 *
 * Accumulators are still float (PCM is mono int16, this is way above
 * the precision we need), and ringing is bounded because we subtract
 * 2π whenever the value crosses it.
 */
static float s_phi0;
static float s_phi1;
static float s_phi2;
...
/*
 * Two-mode generator (turn-taking model, see bsp_qemu_mic_set_mode):
 *
 *   BSP_QEMU_MIC_MODE_SILENCE        -> low-amplitude white noise (±64),
 *                                       keeps the uplink DATA stream alive
 *                                       without tripping VAD as voice.
 *
 *   BSP_QEMU_MIC_MODE_VOICE_ONESHOT  -> play the fixture once from sample 0,
 *                                       then auto-revert to SILENCE and
 *                                       set s_qemu_voice_done=true.
 *
 * The legacy formant-tone synthesizer (kept above as documentation) is
 * unreachable. The "loop the fixture" behaviour from earlier revisions is
 * also retired: turn-taking has the user say one line per turn, not loop.
 */
static volatile bsp_qemu_mic_mode_t s_qemu_mic_mode = BSP_QEMU_MIC_MODE_SILENCE;
static volatile bool                s_qemu_voice_done = true;
static size_t                       s_qemu_voice_cursor;     /* 0..fixture_n in VOICE mode */
static uint32_t                     s_qemu_silence_lcg = 0xC0FFEEu;

void bsp_qemu_mic_set_mode(bsp_qemu_mic_mode_t mode)
{
    if (mode == BSP_QEMU_MIC_MODE_VOICE_ONESHOT) {
        s_qemu_voice_cursor = 0U;
        s_qemu_voice_done   = false;
    }
    s_qemu_mic_mode = mode;
}

bsp_qemu_mic_mode_t bsp_qemu_mic_get_mode(void)
{
    return s_qemu_mic_mode;
}

bool bsp_qemu_mic_voice_done(void)
{
    return s_qemu_voice_done;
}

static inline int16_t qemu_silence_sample(void)
{
    /* xorshift32 -> [-64, +63]. Below STT/VAD energy floor by a wide margin. */
    s_qemu_silence_lcg ^= s_qemu_silence_lcg << 13;
    s_qemu_silence_lcg ^= s_qemu_silence_lcg >> 17;
    s_qemu_silence_lcg ^= s_qemu_silence_lcg << 5;
    return (int16_t)((int32_t)(s_qemu_silence_lcg & 0x7Fu) - 64);
}

static void qemu_mic_fill(int16_t *out, size_t samples)
{
    /* Suppress 'unused' warnings for the legacy formant constants/state - they
     * are intentionally retained as documentation of the previous strategy. */
    (void)s_qemu_mic_lcg;
    (void)s_phi0; (void)s_phi1; (void)s_phi2;

    const size_t fixture_n = (size_t)K_TEST_VOICE_NUM_SAMPLES;
    s_qemu_mic_phase_samples += samples;

    size_t produced = 0U;
    while (produced < samples) {
        if (s_qemu_mic_mode == BSP_QEMU_MIC_MODE_VOICE_ONESHOT && fixture_n > 0U) {
            size_t remaining = fixture_n - s_qemu_voice_cursor;
            size_t need      = samples - produced;
            size_t take      = (remaining < need) ? remaining : need;
            if (take > 0U) {
                memcpy(out + produced,
                       &kTestVoicePcm[s_qemu_voice_cursor],
                       take * sizeof(int16_t));
                produced += take;
                s_qemu_voice_cursor += take;
            }
            if (s_qemu_voice_cursor >= fixture_n) {
                /* Fixture finished -> back to silence for the rest of this
                 * read and all reads thereafter, until the user-loop calls
                 * bsp_qemu_mic_set_mode(VOICE_ONESHOT) again. */
                s_qemu_mic_mode    = BSP_QEMU_MIC_MODE_SILENCE;
                s_qemu_voice_done  = true;
                s_qemu_voice_cursor = 0U;
            }
        } else {
            /* SILENCE: low-amplitude white noise. */
            size_t need = samples - produced;
            for (size_t i = 0U; i < need; ++i) {
                out[produced + i] = qemu_silence_sample();
            }
            produced += need;
        }
    }
...
esp_err_t bsp_microphone_read(void *data, size_t len, size_t *bytes_read, uint32_t timeout_ms)
{
    ...
    if (!s_qemu_mic_inited || data == NULL || len == 0U) {
        return ESP_ERR_INVALID_STATE;
    }
    size_t want_samples = len / sizeof(int16_t);
    if (want_samples == 0U) return ESP_ERR_INVALID_ARG;

    /*
     * Block until we've filled the entire `len` (matches the real PDM driver
     * behaviour and what dialog_uplink_task expects), or until `timeout_ms`
     * is exhausted. The earlier "return after one tick" behaviour caused
     * dlg_ul to spin in its retry loop printing INFO-level "mic fill /
     * mic read pending" lines a hundred times per frame; on QEMU that
     * saturates the UART TX FIFO and pushes uart_write -> task_wdt.
     */
    int16_t *out = (int16_t *)data;
    size_t emitted_samples = 0U;
    TickType_t start_ticks = xTaskGetTickCount();
    TickType_t timeout_ticks = pdMS_TO_TICKS(timeout_ms);

    while (emitted_samples < want_samples) {
        uint64_t avail = qemu_mic_available_samples();
        if (avail > 0U) {
            size_t take = want_samples - emitted_samples;
            if ((uint64_t)take > avail) take = (size_t)avail;
            qemu_mic_fill(out + emitted_samples, take);
            s_qemu_mic_samples_emitted += take;
            emitted_samples += take;
            if (emitted_samples >= want_samples) break;
        }
        if (timeout_ms == 0U) break;
        TickType_t elapsed = xTaskGetTickCount() - start_ticks;
        if (elapsed >= timeout_ticks) break;

        /* Wait for the next frame's worth of samples to accrue, capped by
         * the remaining timeout. We sleep on wall-clock since esp_timer
         * (which paces qemu_mic_available_samples) is wall-clock too. */
        size_t missing = want_samples - emitted_samples;
        uint32_t need_us = (uint32_t)(((uint64_t)missing * 1000000ULL) /
                                       BSP_QEMU_MIC_RATE_HZ);
        uint32_t sleep_ms = (need_us / 1000U) + 1U;
        TickType_t remaining = timeout_ticks - elapsed;
        TickType_t sleep_ticks = pdMS_TO_TICKS(sleep_ms);
        if (sleep_ticks == 0U) sleep_ticks = 1U;
        if (sleep_ticks > remaining) sleep_ticks = remaining;
        vTaskDelay(sleep_ticks);
    }

    if (bytes_read) *bytes_read = emitted_samples * sizeof(int16_t);
    return emitted_samples > 0U ? ESP_OK : ESP_ERR_TIMEOUT;
}

#else /* CONFIG_COLLAR_QEMU_OPENETH */
